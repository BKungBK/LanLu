...
async function inspect(page, route, viewportName, authenticated) {
  ...
  try {
    ...
    return { finalPath: location.pathname, expectedLogin, onLogin, title: document.title, textLength: document.body.innerText.length, expectedMissing: route === "/login" && !onLogin ? ["login redirect"] : [], unnamedInteractive, smallTouchTargets, viewportWidth, scrollWidth, horizontalOverflow: scrollWidth > viewportWidth + 1, navigation: navigation ? { domContentLoaded: Math.round(navigation.domContentLoadedEventEnd), loadEventEnd: Math.round(navigation.loadEventEnd), transferSize: navigation.transferSize } : null };
    }, { route, authenticated });
  } catch {
    await page.waitForLoadState("domcontentloaded").catch(() => {});
    inspection = await page.evaluate(({ route, authenticated }) => ({ finalPath: location.pathname, expectedLogin: !authenticated && route !== "/login", onLogin: location.pathname === "/login", title: document.title, textLength: document.body.innerText.length, expectedMissing: [], unnamedInteractive: [], smallTouchTargets: [], viewportWidth: document.documentElement.clientWidth, scrollWidth: document.documentElement.scrollWidth, horizontalOverflow: document.documentElement.scrollWidth > document.documentElement.clientWidth + 1, navigation: null }), { route, authenticated });
  }
