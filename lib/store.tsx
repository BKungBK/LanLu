...
export function LanluProvider({ children }: { children: React.ReactNode }) {
  ...
  const loadForUser = useCallback(async (currentUser: { id: string; email?: string }) => {
    ...
    const [menuResult, ingredientResult, lotResult, movementResult, recipeResult, salesResult, recommendationResult] = await Promise.all([
      supabase.from("menu_items").select("id, name, price, active, archived_at, menu_categories(name)").eq("shop_id", shopId).order("created_at"),
      supabase.from("ingredients").select("id, name, unit, supplier, active, reorder_point, unit_cost, purchase_package_unit, purchase_package_count, purchase_content_quantity, purchase_content_unit, purchase_price, purchase_conversion_factor").eq("shop_id", shopId).order("created_at"),
      ...
      supabase.from("recipes").select("id, menu_item_id, version, updated_at, archived_at, recipe_lines(ingredient_id, quantity)").eq("shop_id", shopId).order("version", { ascending: false }),
    ...
    const menuItems = (menuResult.data ?? []).map((menu: any) => { const category = Array.isArray(menu.menu_categories) ? menu.menu_categories[0]?.name : menu.menu_categories?.name; return { id: menu.id, name: menu.name, category: (category || "อื่น ๆ") as MenuCategory, price: number(menu.price), active: Boolean(menu.active), archivedAt: menu.archived_at ?? undefined }; });
  ...
  const archiveCatalogItem = useCallback(async (kind: "ingredient" | "menu", id: string): Promise<CatalogActionResult> => {
    if (!state.shop.id) return { ok: false, message: "ยังไม่มีร้าน", entity: kind, action: "archive" };
    const { error: archiveError } = await supabase.rpc("archive_catalog_item", { target_shop_id: state.shop.id, target_kind: kind, target_id: id });
    if (archiveError) return { ok: false, message: messageForError(archiveError, "เก็บรายการออกจากระบบไม่สำเร็จ"), entity: kind, action: "archive" };
    await refresh();
    return {
      ok: true,
      message: kind === "ingredient" ? "เก็บวัตถุดิบออกจากรายการแล้ว · ledger และสูตรเดิมยังอยู่" : "เก็บเมนูออกจากรายการแล้ว · ยอดขายและสูตรเดิมยังอยู่",
      entity: kind,
      action: "archive",
      impact: kind === "ingredient" ? ["ledger เดิมไม่ถูกลบ", "สูตรเดิมไม่ถูกลบ", "ไม่แสดงใน Quick capture"] : ["sales snapshot เดิมไม่ถูกลบ", "สูตรเดิมไม่ถูกลบ", "ไม่แสดงใน Quick capture"],
    };
  }, [refresh, state.shop.id, supabase]);
  const restoreCatalogItem = useCallback(async (kind: "ingredient" | "menu", id: string): Promise<CatalogActionResult> => {
    if (!state.shop.id) return { ok: false, message: "ยังไม่มีร้าน", entity: kind, action: "restore" };
    const { error: restoreError } = await supabase.rpc("restore_catalog_item", { target_shop_id: state.shop.id, target_kind: kind, target_id: id });
    if (restoreError) return { ok: false, message: messageForError(restoreError, "เลิกเก็บรายการไม่สำเร็จ"), entity: kind, action: "restore" };
    await refresh();
    return {
      ok: true,
      message: kind === "ingredient" ? "เลิกเก็บวัตถุดิบแล้ว · กลับมาใช้ในสูตรและ Quick capture ได้" : "เลิกเก็บเมนูแล้ว · กลับมาใช้ใน Quick capture ได้",
      entity: kind,
      action: "restore",
  ...
  const archiveRecipe = useCallback(async (menuItemId: string): Promise<Result> => { if (!state.shop.id) return { ok: false, message: "ยังไม่มีร้าน" }; const { error: archiveError } = await supabase.rpc("archive_recipe", { target_shop_id: state.shop.id, target_menu_item_id: menuItemId }); if (archiveError) return { ok: false, message: messageForError(archiveError, "เก็บสูตรออกจากระบบไม่สำเร็จ") }; await refresh(); return { ok: true, message: "เก็บสูตรเวอร์ชันล่าสุดแล้ว" }; }, [refresh, state.shop.id, supabase]);
