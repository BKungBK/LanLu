...
export function IngredientSettings() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (!name.trim() || quantity < 0 || unitCost < 0) return;
    setPending(true); setFeedback("");
    const hasPurchase = Boolean(packageUnit.trim() && packageCount > 0 && contentQuantity > 0 && purchasePrice >= 0);
    const result = editingId
      ? await updateIngredient({ id: editingId, name: name.trim(), unit, unitCost: calculatedUnitCost ?? unitCost, purchase })
      : await addIngredient({ name: name.trim(), unit, quantityOnHand: quantity, reorderPoint: 0, unitCost: calculatedUnitCost ?? unitCost, purchase: hasPurchase ? purchase : undefined, openingExpiry });
  ...
  return <>
    <PageHeader eyebrow="Catalog" title="วัตถุดิบและต้นทุน" description="จัดการวัตถุดิบและต้นทุน · จุดหลักในการจัดการอยู่ที่หน้า Inventory" action={<Link href="/inventory" className="button button-soft"><IconToolsKitchen2 size={15} />ไปสต๊อก</Link>} />
    <div className="settings-grid">
      ...
      <SectionCard className="form-card" title={editingId ? "แก้ไขวัตถุดิบ" : "เพิ่มวัตถุดิบ"} description={editingId ? "แก้ master data ได้โดยไม่เขียนทับ movement เดิม" : "ยอดเริ่มต้นจะสร้าง receipt lot พร้อม audit trail"}>
        <form className="form-grid" onSubmit={submit}>
          ...
          <div className="form-field full"><CreatableSelect id="ingredient-unit" label="หน่วยตัดสต๊อก" value={unit} options={units} onChange={setUnit} onCreate={(value) => setUnit(value)} /><small className="form-hint">หน่วยที่ใช้ทุกครั้งที่รับของ ขาย ทิ้ง หรือปรับยอด เช่น ml, g หรือ ขวด</small></div>
          <div className="form-field"><label htmlFor="ingredient-cost">ต้นทุนต่อหน่วย (ถ้ารู้)</label><input id="ingredient-cost" className="text-input" type="number" min="0" step="0.000001" value={calculatedUnitCost ?? unitCost} onChange={(event) => setUnitCost(Number(event.target.value))} readOnly={calculatedUnitCost !== null} /><small className="form-hint">คำนวณจากแพ็กซื้อเมื่อข้อมูลครบ</small></div>
          ...
          {!editingId && <><div className="form-field"><label htmlFor="ingredient-qty">มีอยู่ตอนนี้</label><input id="ingredient-qty" className="text-input" type="number" min="0" step="0.001" value={quantity} onChange={(event) => setQuantity(Number(event.target.value))} /><small className="form-hint">จำนวนที่มีในร้านตอนเริ่มใช้ระบบ</small></div><DateField id="ingredient-expiry" label="วันหมดอายุยอดเริ่มต้น (ถ้ามี)" value={openingExpiry} onChange={setOpeningExpiry} /></>}
