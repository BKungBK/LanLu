...
const templates: Record<CatalogDraftKind, Array<Record<string, unknown>>> = {
  ingredient: [{ name: "นมสด", unit: "ลิตร", supplier: "", unitCost: 50, reorderPoint: 2, quantityOnHand: 5, expiresOn: "", packageUnit: "ขวด", packageCount: 1, contentQuantity: 1000, contentUnit: "ml", purchasePrice: 50 }],
...
function DraftRow({ kind, row, idPrefix, onChange }: { kind: CatalogDraftKind; row: Record<string, unknown>; idPrefix: string; onChange: (key: string, value: unknown) => void }) {
  const fields: Array<[string, string]> = kind === "ingredient" ? [["name", "ชื่อ"], ["unit", "หน่วย"], ["supplier", "Supplier"], ["unitCost", "ต้นทุน/หน่วย"], ["reorderPoint", "จุดสั่งซื้อ"], ["quantityOnHand", "ยอดเริ่มต้น"], ["packageUnit", "หน่วยแพ็ก"], ["packageCount", "จำนวนแพ็ก"], ["contentQuantity", "ปริมาณ/แพ็ก"], ["contentUnit", "หน่วยในแพ็ก"], ["purchasePrice", "ราคาแพ็ก"], ["conversionFactor", "อัตราแปลง (ถ้าจำเป็น)"]] : kind === "menu" ? [["name", "ชื่อเมนู"], ["category", "หมวด"], ["price", "ราคาขาย"], ["active", "เปิดขาย"]] : [["menuName", "ชื่อเมนู"], ["ingredientName", "ชื่อวัตถุดิบ"], ["quantity", "ปริมาณ"], ["unit", "หน่วย"]];
  return <div className="draft-row"><div className="draft-row-grid">{fields.map(([key, label]) => <div className="form-field" key={key}><label htmlFor={`draft-${idPrefix}-${key}`}>{label}</label>{key === "active" ? <input id={`draft-${idPrefix}-${key}`} type="checkbox" checked={row[key] !== false && row[key] !== "false"} onChange={(event) => onChange(key, event.target.checked)} /> : <input id={`draft-${idPrefix}-${key}`} className="text-input" value={String(row[key] ?? "")} onChange={(event) => onChange(key, ["price", "unitCost", "reorderPoint", "quantityOnHand", "quantity", "packageCount", "contentQuantity", "purchasePrice", "conversionFactor"].includes(key) ? Number(event.target.value) : event.target.value)} />}</div>)}</div></div>;
}
