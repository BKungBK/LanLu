...
export function ResetPasswordPage() {
  ...
  return <div className="auth-page auth-page-centered"><section className="auth-card"><div className="auth-brand"><span className="brand-mark">LL</span><div><div className="brand-name">LanLu</div><div className="brand-sub">ร้านรู้</div></div></div><h2>ตั้ง password ใหม่</h2><p>ตั้งรหัสใหม่เพื่อกลับเข้าใช้งานร้าน</p><form className="auth-form" onSubmit={submit}><div className="form-field"><label htmlFor="new-password"><IconLock size={14} /> Password ใหม่</label><input suppressHydrationWarning id="new-password" className="text-input" type="password" minLength={8} autoComplete="new-password" value={password} onChange={(event) => setPassword(event.target.value)} required /></div><div className="form-field"><label htmlFor="confirm-password"><IconLock size={14} /> ยืนยัน password</label><input suppressHydrationWarning id="confirm-password" className="text-input" type="password" minLength={8} autoComplete="new-password" value={confirm} onChange={(event) => setConfirm(event.target.value)} required /></div>{error && <div className="auth-error" role="alert">{error}</div>}<button type="submit" className="button button-primary" disabled={pending}>{pending ? "กำลังบันทึก…" : <><IconCheck size={16} />บันทึก password ใหม่</>}</button></form></section></div>;
}
