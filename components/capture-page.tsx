  ...
  return <>
    ...
    <div className="capture-layout">
      <SectionCard className="capture-panel" title="เริ่มบันทึกข้อมูล" description="เลือกวันที่ธุรกิจ แล้วระบบจะคำนวณ stock จากสูตรให้อัตโนมัติ">
        <div className="capture-mode" role="tablist" aria-label="ประเภทการบันทึก">{modes.map(({ id, label, icon: Icon }) => <button type="button" role="tab" id={`capture-tab-${id}`} aria-controls={`capture-panel-${id}`} aria-selected={mode === id} key={id} className={`mode-button ${mode === id ? "active" : ""}`} onClick={() => changeMode(id)}><span className="mode-icon"><Icon size={16} /></span><span>{label}</span></button>)}</div>
        <div className="date-field"><DateField id="business-date" label="วันที่ข้อมูล" value={businessDate} onChange={setBusinessDate} /></div>
        {mode === "sales" ? <div role="tabpanel" id="capture-panel-sales" aria-labelledby="capture-tab-sales">
        ...
        </div> : <div role="tabpanel" id={`capture-panel-${mode}`} aria-labelledby={`capture-tab-${mode}`}>
