...
export function LoginPage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    setPending(false);
    if (result.error) {
      setError(result.error.message.includes("Invalid login") ? "email หรือ password ไม่ถูกต้อง" : result.error.message);
      return;
    }
  ...
  return <div className="auth-page">
    ...
    <section className="auth-card" aria-labelledby="auth-title">
      ...
      <form className="auth-form" onSubmit={submit} noValidate>
        ...
        <button type="submit" className="button button-primary" disabled={pending}>{pending ? "กำลังตรวจสอบ…" : mode === "login" ? <>เข้าสู่ Dashboard <IconArrowRight size={16} /></> : <>สร้างบัญชี <IconArrowRight size={16} /></>}</button>
      </form>
